...
**Use case**: Providing reliable peerid in peerjs and more generally helping in webrtc app (such as webtorrent) for more trustable p2p human identification, to bypass, or reduce, the need of a server (such as peerjs-server). eg. this could become a "iiaom" plugin using "groups" and "webrtc" on wp, where the url of the page has hash(iiaom), the page is owned by the available one registering the ip in the page and page is visible only to visitors in the specific group created by the owner of the page, hence as a new feature in creating groups in wp, or for p2p by iiaom on xmpp or webrtc with tor or other means, consists of: 
